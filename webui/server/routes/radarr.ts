...
router.post('/', async (req, res) => {
    try {
        const repo = getRepository(RadarrInstance);
        // Explicitly destructure to avoid unsafe assignment
        const {
        ...
        const instance = repo.create({
            name, baseUrl, apiKey, originalProfile, dubProfile, tagName,
            originalLanguages: JSON.stringify(originalLanguages || []), // Ensure it's stringified
        ...
        console.error('[Radarr Route] Error creating instance:', error);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (apiKey) instance.apiKey = apiKey; // Only update if provided
...
// Proxy routes to fetch metadata
